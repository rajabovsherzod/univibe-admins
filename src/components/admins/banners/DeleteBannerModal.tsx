...
import { useDeleteBanner } from "@/hooks/api/use-banners-admin";
import type { BannerManagement } from "@/types/admins/banners";

interface DeleteBannerModalProps {
  item: BannerManagement;
  onClose: () => void;
}

export function DeleteBannerModal({ item, onClose }: DeleteBannerModalProps) {
  const [confirmName, setConfirmName] = useState("");
  const del = useDeleteBanner();

  const isMatching = confirmName.trim() === item.title;
      ...
      <div className="flex flex-col gap-4 pt-2">
        <div className="rounded-lg bg-secondary p-4 ring-1 ring-secondary ring-inset">
          <p className="text-sm text-secondary">
            Haqiqatan ham <strong className="font-semibold text-primary">«{item.title}»</strong> bannerni o&apos;chirmoqchimisiz?
            Tizimdagi bu ma&apos;lumot butunlay o&apos;chib ketadi.
          </p>
        </div>

        <Input
          label="Tasdiqlash uchun banner nomini kiriting"
