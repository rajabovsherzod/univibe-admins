...
const TableCardRoot = ({ children, className, size = "md", ...props }: HTMLAttributes<HTMLDivElement> & { size?: "sm" | "md" }) => {
  return (
    <TableContext.Provider value={{ size }}>
      <div {...props} className={cx("overflow-hidden rounded-xl bg-primary shadow-md ring-1 ring-secondary", className)}>
        {children}
      </div>
...
const TableCardHeader = ({ title, badge, description, contentTrailing, className }: TableCardHeaderProps) => {
  ...
  return (
    <div
      className={cx(
        "relative flex flex-col items-start gap-4 border-b border-white/20 bg-brand-solid px-4 md:flex-row",
...
const TableHeader = <T extends object>({ columns, children, bordered = true, className, ...props }: TableHeaderProps<T>) => {
  ...
  return (
    <AriaTableHeader
      ...
      className={(state) =>
        cx(
          "relative bg-brand-solid",
          size === "sm" ? "h-9" : "h-11",

          // Row border—using an "after" pseudo-element to avoid the border taking up space.
          bordered &&
          "[&>tr>th]:after:pointer-events-none [&>tr>th]:after:absolute [&>tr>th]:after:inset-x-0 [&>tr>th]:after:bottom-0 [&>tr>th]:after:h-px [&>tr>th]:after:bg-white/20 [&>tr>th]:focus-visible:after:bg-transparent",
      ...
      {(state) => (
        <AriaGroup className="flex items-center gap-1">
          <div className="flex items-center gap-1">
            {label && <span className="text-xs font-semibold whitespace-nowrap text-white">{label}</span>}
