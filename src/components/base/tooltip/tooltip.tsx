...
interface TooltipProps extends AriaTooltipTriggerComponentProps, Omit<AriaTooltipProps, "children"> {
  ...
  delay?: number;
  /**
   * Background color theme for the tooltip.
   * @default "default"
   */
...
const arrowColorStyles = {
  default: "fill-bg-primary-solid",
  brand: "fill-brand-solid",
  success: "fill-success-solid",
  error: "fill-error-solid",
};
    ...
    <AriaTooltipTrigger {...{ trigger, delay, closeDelay, isDisabled, isOpen, defaultOpen, onOpenChange }}>
      ...
      >
        {({ isEntering, isExiting }) => (
          <div
            className={cx(
              "z-50 flex max-w-xs origin-(--trigger-anchor-point) flex-col items-start gap-1 rounded-lg px-3 shadow-lg will-change-transform",
              colorStyles[color],
